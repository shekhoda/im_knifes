(() => {
    ...
    function rangeInit() {
        ...
        if (rangeItems) rangeItems.forEach((rangeItem => {
            ...
            if (!item || !fromValue || !toValue) return;
            const inputsRange = [ fromValue, toValue ];
            initialize(item, {
                range: {
    ...
    function initSliders() {
        ...
        if (document.querySelector(".products-slider__slider")) new swiper_core_Swiper(".products-slider__slider", {
        ...
        if (document.querySelector(".products-new__slider")) new swiper_core_Swiper(".products-new__slider", {
            modules: [ Pagination, Navigation, Autoplay ],
            observer: true,
            watchOverflow: true,
            observeParents: true,
        ...
        if (document.querySelector(".thumbs-images")) {
            const thumbsSwiper = new swiper_core_Swiper(".thumbs-images", {
                modules: [ Pagination, Navigation, Autoplay, Thumb ],
                observer: true,
                watchOverflow: true,
                observeParents: true,
                slidesPerView: 4,
    ...
    if (menuBlocks.length) menuBlocks.forEach((menuBlock => {
        const menuBlockItems = menuBlock.querySelectorAll(".sub-menu-catalog__link-category").length;
    ...
    function documentActions(e) {
        ...
        if (targetElement.closest("[data-parent]")) {
            ...
            } else console.log("Oй, нет такого подменю");
            e.preventDefault();
        }
